...
public class LogMgr implements Iterable<BasicLogRecord> {
	/**
	 * The location where the pointer to the last integer in the page is. A value of
	 * 0 means that the pointer is the first value in the page.
	 */
	 ...
	 * exist, it is created with an empty first block. This constructor depends on a
	 * {@link FileMgr} object that it gets from the method
	...
	/**
	 * Returns the LSN of the most recent log record. As implemented, the LSN is the
	 * block number where the record is stored. Thus every log record in a block has
	 * the same LSN.
	 * 
	...
	/**
	 * Sets up a circular chain of pointers to the records in the page. There is an
	...
	/**
	 * Update mypage with the page from Bufferpool
	 * @author abhardw3, achauha3, kdpandya, nkapadi, pjmandle
	 * */
	public void setPage(Buffer pin) {
		this.mypage = pin.getContents();
	}
}
